...
async function generateProof() {
    try {
        // Step 1: Calculate the witness
        await snarkjs.wtns.calculate(
            input,
            path.join(baseDir, '../../utils/circuit_js/circuit.wasm'),
            path.join(baseDir, '../../utils/circuit.wtns')
        );

        // Step 2: Generate the proof and public signals using the witness
...
async function verifyProofOnChain(proof, publicSignals) {
    try {
        ...
        const verifierAddress = '0x2FB04449A4d4C49bdaD339757A740f98E03A1f2b'; // Replace with actual deployed verifier address
        const verifierABI = [
...
async function main() {
    ...
    const { a, b, c, publicSignals } = proofData;

    const isValid = await verifyProofOnChain({ a, b, c }, publicSignals);
